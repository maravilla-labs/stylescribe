...
StyleScribe - A design system built with Stylescribe
...
**ALWAYS use `ds-` prefix in source SCSS files.** The configured prefix (`styl-`) is applied automatically at build time.
...
StyleScribe can generate screenshots for components, blocks, pages, and documentation. Screenshots appear as thumbnails in search results.

### Generating Screenshots

```bash
# Generate all screenshots (requires Playwright - auto-installed on first run)
...
| Type | Source | Output |
|------|--------|--------|
| `components` | Component previews | `static/screenshots/component-{name}.png` |
| `blocks` | UI block previews | `static/screenshots/block-{name}.png` |
| `pages` | Full page previews | `static/screenshots/page-{name}.png` |
| `docs` | Documentation pages | `static/screenshots/page-{slug-with-dashes}.png` |

### Search Integration

Screenshots automatically appear as thumbnails in search results when available. The search index includes screenshot paths for all indexed content types.

### Workflow

1. Run `stylescribe docs` to generate the site
2. Run `stylescribe screenshots` to capture screenshots
3. Screenshots are cached - only changed items are regenerated
